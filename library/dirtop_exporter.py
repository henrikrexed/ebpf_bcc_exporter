...
histogram_readKB = Gauge('ebpf_dirtop_rbytes', 'Read Kbytes per directory',['path'])
histogram_writeKB = Gauge('ebpf_dirtop_wkbytes', 'Write Kbyte per directory',['path'])
...
while 1:
    ...
    for node_id in reads:
        ...
        histogram_readKB.labels(inodes_to_path[node_id]).set(reads_Kb[node_id])
        histogram_writeKB.labels(inodes_to_path[node_id]).set(writes_Kb[node_id])
        line += 1
        if line >= maxrows:
            break
