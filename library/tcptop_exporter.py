...
parser.add_argument("port", type=int, default=default_port,
    help="port to expose the prometheus metrics")
...
histogram_receivedKB = Gauge('ebpf_tcptop_rbytes', 'Throughput by host and port',['command','LadressIPV4','RadressIPV4'])
histogram_TransmittedKB = Gauge('ebpf_tcptop_Tkbytes', 'Throughput by host and port',['command','LadressIPV4','RadressIPV4'])
histogram_receivedKBv6 = Gauge('ebpf_tcptop_rkbytes', 'Throughput by host and port ipv6',['command','LadressIPV4','RadressIPV4'])
histogram_TransmittedKBv6 = Gauge('ebpf_tcptop_tkbytes', 'Throughput by host and port ipv6',['command','LadressIPV4','RadressIPV4'])
...
while i != args.count and not exiting:
    ...
    for k, (send_bytes, recv_bytes) in sorted(ipv4_throughput.items(),
        ...
        histogram_receivedKB.labels(pid_to_comm(k.pid), k.laddr + ":" + str(k.lport),k.daddr + ":" + str(k.dport)).set(int(recv_bytes / 1024))
        histogram_TransmittedKB.labels(pid_to_comm(k.pid), k.laddr + ":" + str(k.lport),k.daddr + ":" + str(k.dport)).set( int(send_bytes / 1024))
    ...
    for k, (send_bytes, recv_bytes) in sorted(ipv6_throughput.items(),
        ...
        print("%-6d %-12.12s %-32s %-32s %6d %6d" % (k.pid,
            ...
            k.daddr + ":" + str(k.dport),
            int(recv_bytes / 1024), int(send_bytes / 1024)))
        histogram_receivedKBv6.labels(pid_to_comm(k.pid), k.laddr + ":" + str(k.lport), k.daddr + ":" + str(k.dport)).set(
                                    int(recv_bytes / 1024))
        histogram_TransmittedKBv6.labels(pid_to_comm(k.pid), k.laddr + ":" + str(k.lport), k.daddr + ":" + str(k.dport)).set(
                                       int(send_bytes / 1024))
