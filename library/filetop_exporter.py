...
#
# USAGE: filetop_exporter.py [-h] [-C] [-r MAXROWS] [interval] [count]
#
...
histogram_readKB = Gauge('ebpf_filetop_rbytes', 'writes by file',['command','file'])
histogram_writedKB = Gauge('ebpf_filetop_wkbytes', 'writes by file',['command','file'])
...
while 1:
    ...
    for k, v in reversed(sorted(counts.items(),
        ...
        histogram_readKB.labels(k.comm.decode('utf-8', 'replace'),name).set( v.rbytes / 1024)
        histogram_writedKB.labels(k.comm.decode('utf-8', 'replace'), name).set(v.wbytes / 1024)
        line += 1
        if line >= maxrows:
            break
    counts.clear()
