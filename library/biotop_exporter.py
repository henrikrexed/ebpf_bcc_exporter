...
while 1:
    ...
    for k, v in reversed(sorted(counts.items(),
        ...
        histogram_io.labels(k.name.decode('utf-8', 'replace'),"W" if k.rwflag else "R",diskname).set(v.io)
        histogram_kbytes.labels(k.name.decode('utf-8', 'replace'), "W" if k.rwflag else "R", diskname).set(v.bytes / 1024)
        histogram_avglatency_ms.labels(k.name.decode('utf-8', 'replace'), "W" if k.rwflag else "R", diskname).set(
            avg_ms)
        line += 1
        if line >= maxrows:
            break
